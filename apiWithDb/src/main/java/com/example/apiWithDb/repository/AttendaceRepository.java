...
public interface AttendaceRepository extends JpaRepository<AttendanceRecord, Long> {
    ...
    @Query("SELECT a FROM AttendanceRecord a WHERE a.employee.department.id = :departmentId AND a.Date = :attendanceDate")
    List<AttendanceRecord> findAttendanceRecordsByDepartmentIdAndDate(@Param("departmentId") Long departmentId,
                                                                      @Param("attendanceDate") LocalDate attendanceDate);

    @Query("SELECT ar FROM AttendanceRecord ar WHERE ar.Date = ?1 AND ar.employee.id = ?2")
